...
def prepare_pipeline():
    ...
    df = build_gssi_pipeline(features)
...
def get_dashboard():
    ...
    history = [
        {
            "week": str(r["week"].date()) if hasattr(r["week"], "date") else str(r["week"])[:10],
            "gssi": round(float(r["gssi"]), 4),
        }
        for _, r in df[["week", "gssi"]].iterrows()
    ]

    components = [
        {
            "week": str(r["week"].date()) if hasattr(r["week"], "date") else str(r["week"])[:10],
            "oil": round(float(r["oil_price"]), 4),
            "volatility": round(float(r["market_volatility"]), 4),
            "freight": round(float(r["freight_cost"]), 4),
            "supplier": round(float(r["supplier_delay"]), 4),
            "inventory": round(float(r["inventory_stress"]), 4),
        }
        for _, r in df[["week", "oil_price", "market_volatility", "freight_cost", "supplier_delay", "inventory_stress"]].iterrows()
    ]
    ...
    return {
        ...
        "summary": {
            "rule_based_summary": summary_data["rule_based_summary"],
            "ai_summary": summary_data["ai_summary"],
            "ai_explanation": summary_data["ai_explanation"],
            "ai_recommendation": summary_data["ai_recommendation"],
        },
        "history": history,
